...
init_wheel_ang_vel = orb_vel/sqrt(2); %initial reaction wheel angular velocity
...
[lat, long] = Orbit3D(RA_ascend, arg_per, true_anom, inclin, semi_major, eccent, time_step, num_orb);
...
% Iterate through, creating a matrix of mag field components in orbital frame
for i = 0:(time_step/60):sim_time
    ...
    % Gives the individual vectors for each timestep
    rotated_B_vec = ned_orb_matrix * B_field_NED(NED_matrix_row,:)';
    % Creates the matrix of orbital components, adding a row each iteration
    B_field_orb = [B_field_orb; rotated_B_vec'];
    % Update the row counter so the matrix multiplication updates each time
...
%       -cosd(lat)*cos(vern_ang)  -cosd(lat)*sind(vern_ang) -sind(lat)]; %direction cosine matrix
