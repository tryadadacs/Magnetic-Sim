...
% If r
function [LatSSP, LongSSP] = Orbit3D(RA_ascend, arg_per, true_anom, inclin, semi_major, eccent, time_step, num_orb)
%% INTRO
...
%% TIME
norb = num_orb;  % number of orbits
t0   = 0;                                        % initial time          [s]
tf   = norb*T;                                   % final   time          [s]  
step = time_step; % time step             [s]
...
if ((grp==2)||(grp==3))
    ...
    %comment here to remove planisphere
    %{
    ...
    %end comment to remove planisphere here
    %}
end
%
end